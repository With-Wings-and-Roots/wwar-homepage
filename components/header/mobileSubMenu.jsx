        ...
        .map((subItem, index) => {
          return (
            <div key={index} className='hover:text-wwr_white'>
              <Link href={isExternalLink(subItem.url) ? subItem.url : createLocalLink(subItem.url)} target={isExternalLink(subItem.url) ? '_blank' : '_self'} rel={isExternalLink(subItem.url) ? 'noreferrer noopener': ''} className='min-w-max'>
