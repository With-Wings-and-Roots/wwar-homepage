...
const AllFilmsTemplate = async ({ data, lang = 'en' }) => {
  const films = await getAllFilms(lang);
  const filmProductionTypes = await getFilmProductionTypes(lang);
  const filmLanguages = await getFilmLanguages(lang);
  const filmTypes = await getFilmTypes(lang);
