      ...
      <section className='relative w-full'>
        <div className='max-w-[1180px] mx-auto pt-[72px] px-8 md:px-16 xl:px-0 grid md:grid-cols-2 gap-12 items-center'>
          ...
          <div className='max-w-[560px]'>
            <h1 className='text-[32px] leading-[1.1] font-bold'>
              {data.title.rendered}{' '}
            </h1>
...
'
            >
              {acf?.ctas?.[0]?.primary_ctas && (
                <a
                  href={acf.ctas[0].primary_ctas.url}
                  className='
h-[50px]
px-7
flex
...
'
                >
                  {acf.ctas[0].primary_ctas.title}
                </a>
              )}

              {acf?.ctas?.[1]?.primary_ctas && (
                <a
                  href={acf.ctas[1].primary_ctas.url}
                  className='
text-wwr_teal
underline
underline-offset-4
          ...
          {acf.what_we_offer?.map((item, i) => (
            ...
            >
              ...
              <div className='p-[24px]'>
                ...
                />
                <span
                  className='mt-4 inline-block text-wwr_teal text-[16px]'
                  dangerouslySetInnerHTML={{
                    __html: `${item.link?.title || 'Learn More'} →`,
                  }}
                />
              </div>
            </a>
          ...
          {featuredWorkshops?.[0] && (
            <Link
              ...
              href={createLocalLink(
                `/${lang}/workshops/${featuredWorkshops[0].slug}`
              )}
      ...
      <div className="grid md:grid-cols-4 gap-6">
        {acf.steps.map((step, i) => (
          <div
            ...
            style={{
              background: [
                'linear-gradient(135deg, #3A6F73 0%, #2F5D61 100%)',
                'linear-gradient(135deg, #3A6F73 0%, #284E52 100%)',
                'linear-gradient(135deg, #355F63 0%, #24484B 100%)',
                'linear-gradient(135deg, #2F5D61 0%, #203F42 100%)',
              ][i % 4],
            }}
          >
        ...
        <div className='grid md:grid-cols-3 gap-6 mt-10'>
          {(acf.resouces || []).map((item, i) => {
            ...
            const slug = item.page_link
              ? new URL(item.page_link).pathname
                  .split('/')
                  .filter(Boolean)
                  .pop()
              : '';
