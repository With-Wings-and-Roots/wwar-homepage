...
import VisualStrip from '../common/visualStrip';
...
const SingleFilmTemplate = async ({ data, film, params, relatedSlug }) => {
  const acf = film?.acf || {};
  const team = acf?.team || [];
  ...
  const [allPersons, topics] = await Promise.all([
    getAllPersons(),
  ...
  const mediaIds = [
    ...new Set([
      ...relatedStories.map((e) => e?.featured_media).filter(Boolean),

      ...formattedTeams
        .map((s) => s?.member?.acf?.profile_icon)
        .filter(Boolean),
    ]),
  ];
  ...
  const deepSlug = params.deepSlug || '';

  return (
    <div className='flex flex-col gap-24 -mt-20 mb-20'>
      {relatedSlug && !!relatedStories?.find((s) => s.slug === relatedSlug) && (
        <StoryPageComponent
          lang={params.lang}
          paramsStory={relatedSlug}
          stories={relatedStories}
          topics={topics}
          allMedia={allMedia}
          allPersons={allPersons}
          baseLink={createLocalLink(
            `/${params.lang}/${data?.slug}/${deepSlug}/`
          )}
          closeLink={createLocalLink(
            `/${params.lang}/${data?.slug}/${deepSlug}/`
          )}
        />
      )}
        ...
        <section id='trailer' className='px-8 md:px-16 xl:px-48'>
          <div className='max-w-4xl'>
            <h2 className='text-2xl lg:text-4xl font-medium mb-6'>
              {params.lang === 'en' ? 'Watch Trailer' : 'Trailer ansehen'}
            </h2>
      ...
      {acf.short_synopsis && (
        <section className=' px-8 md:px-16 xl:px-48'>
          <h2 className='text-2xl lg:text-4xl font-medium mb-6'>
            {params.lang === 'en' ? 'About the Film' : 'Über den Film'}
          </h2>
      ...
      {galleryMedia.length > 0 && (
        <section className='px-8 md:px-16 xl:px-48 py-16'>
          <h2 className='text-2xl lg:text-4xl font-medium mb-6'>
            {params.lang === 'en' ? `Film Stills` : 'Filmszenen'}
          </h2>
          <div className='w-full md:w-[70%] mx-auto'>
            <VisualStrip acf={acf} lang={params.lang} />
          </div>
        </section>
      )}
      ...
      {peopleMedia.length > 0 && (
        <section className='px-8 md:px-16 xl:px-48 '>
          <h2 className='text-2xl lg:text-4xl font-medium mb-6'>
            {params.lang === 'en' ? `People in the Film` : 'Personen im Film'}
          </h2>
      ...
      {(acf?.award_list?.length > 0 ||
        ...
        <section className='bg-wwr_yellow_orange pt-6'>
          <section className='px-8 md:px-16 xl:px-48 '>
            ...
            {acf?.award_laurals?.length > 0 && (
              <h2 className='text-2xl lg:text-4xl font-medium pt-12 mb-6'>
                {params.lang === 'en' ? 'Laurels' : 'Lorbeeren'}
              </h2>
            )}
            {acf?.award_laurals?.length > 0 && (
            ...
            {acf?.screening_list?.length > 0 && (
              <h2 className='text-2xl lg:text-4xl font-medium pt-12 mb-6'>
                {params.lang === 'en' ? 'Screenings' : 'Vorführungen'}
              </h2>
            )}
            {acf?.screening_list?.length > 0 && (
        ...
        <section className='px-8 md:px-16 xl:px-48'>
          <h2 className='text-2xl lg:text-4xl font-medium mb-8'>
            {acf?.avaiability_text ||
              (params.lang === 'en' ? 'Where to Watch' : 'Wo ansehen')}
          </h2>
      ...
      {formattedTeams.length > 0 && (
        <section className='px-8 md:px-16 xl:px-48'>
          <h2 className='text-2xl lg:text-4xl font-medium mb-8'>
            {acf?.avaiability_text || (params.lang === 'en' ? 'Team' : 'Team')}
          </h2>
      ...
      {acf.related_stories?.length > 0 && (
        <section className='px-8 md:px-16 xl:px-48 py-12'>
          <h2 className='text-2xl lg:text-4xl font-medium mb-6'>
            {params.lang === 'en' ? 'Related Stories' : 'Verwandte Geschichten'}
          </h2>
