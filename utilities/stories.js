...
export async function getAllStories(lang = 'en') {
  return await fetchAllData(
    `${process.env.CMS_URL}/wp-json/wp/v2/story?lang=${lang}`
  );
}

export async function fetchAllTopics(lang = 'en') {
  return await fetchAllData(
    `${process.env.CMS_URL}/wp-json/wp/v2/story_topic?per_page=100&lang=${lang}`
  );
}

export async function getAllMedia(lang = 'en') {
  return await fetchAllData(
    `${process.env.CMS_URL}/wp-json/wp/v2/media?per_page=100&lang=${lang}`
  );
}

export async function getAllPersons() {
  return await fetchAllData(
    `${process.env.CMS_URL}/wp-json/wp/v2/person?lang=en&per_page=100`
  );
}

export async function getStoryMedia(lang = 'en', slug) {
  const [data] = fetchAllData(
    `${process.env.CMS_URL}/wp-json/wp/v2/story?lang=${lang}&slug=${slug}`
  );
...
export async function getStoryMediaByMediaId(lang, mediaId) {
  const data = await fetchAllData(
    `${process.env.CMS_URL}/wp-json/wp/v2/media/${mediaId}?lang=${lang}`
  );
...
export async function getTopicStories(lang, topicId) {
  return await fetchAllData(
    `${process.env.CMS_URL}/wp-json/wp/v2/story?story_topic=${topicId}&lang=${lang}&per_page=100`
  );
}
...
export const getMenuId = async (_) => {
  const [data] = await fetchAllData(
    `${process.env.CMS_URL}/wp-json/wwarrest/v1/menu`
  );
...
export const getMenuItems = async (id) => {
  return await fetchAllData(
    `${process.env.CMS_URL}/wp-json/wwarrest/v1/menu/${id}`
  );
};
